...
export default class DisplaySingle extends Component {
  ...
  render = () => {
    const { image_url, id, description, timestamp, karma_total, comments} = this.props.photoObj
    return (
      <div>
        ...
        <div>{karma_total}</div>
        <p>{description}</p>
        <CommentFeed imageId={id} comments={comments} setCommentsByPush={() => this.setCommentsByPush()}/>
      </div>
    )
  }
